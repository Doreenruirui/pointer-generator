...
class SummarizationModel(object):
    ...
    def _add_encoder(self, encoder_inputs, seq_len):
        ...
        with tf.variable_scope('encoder'):
            ...
            (encoder_outputs, (fw_st, bw_st)) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, encoder_inputs, dtype=tf.float32, sequence_length=tf.cast(seq_len, tf.int64), swap_memory=True)
            encoder_outputs = tf.concat(concat_dim=2, values=encoder_outputs) # concatenate the forwards and backwards states
    ...
    def _reduce_states(self, fw_st, bw_st):
        ...
        with tf.variable_scope('reduce_final_st'):
            ...
            # Apply linear layer
            old_c = tf.concat(concat_dim=1, values=[fw_st.c, bw_st.c]) # Concatenation of fw and bw cell
            old_h = tf.concat(concat_dim=1, values=[fw_st.h, bw_st.h]) # Concatenation of fw and bw state
    ...
    def _scatter_nd(self, batch_size, batch_nums, extended_vsize, copy_dist):
        shape = [batch_size, extended_vsize]
        copy_prob = tf.reshape(tf.zeros(shape, dtype=tf.float32), [-1])
        linear_indices = tf.reshape(batch_nums, [-1]) * extended_vsize  +  tf.reshape(self._enc_batch_extend_vocab, [-1])
    ...
    def _calc_final_dist(self, vocab_dists, attn_dists):
        ...
        with tf.variable_scope('final_distribution'):
            ...
            vocab_dists_extended = [tf.concat(concat_dim=1, values=[dist, extra_zeros]) for dist in vocab_dists] # list length max_dec_steps of shape (batch_size, extended_vsize)
            ...
            # indices = tf.pack((batch_nums,), axis=2) # shape (batch_size, enc_t, 2)
            # attn_dists_projected = [tf.scatter_nd(indices, copy_dist, shape) for copy_dist in attn_dists] # list length max_dec_steps (batch_size, extended_vsize)
            attn_dists_projected = [self._scatter_nd(self._hps.batch_size, batch_nums,
                                                     extended_vsize, copy_dist) for copy_dist in attn_dists]
                ...
                with tf.variable_scope('loss'):
                    if FLAGS.pointer_gen: # calculate loss from log_dists
                        ...
                        for dec_step, log_dist in enumerate(log_dists):
                            ...
                            losses = gather_2d(-log_dist, batch_nums, targets)
...
def _mask_and_avg(values, padding_mask):
    ...
    dec_lens = tf.reduce_sum(padding_mask, reduction_indices=1) # shape batch_size. float32
